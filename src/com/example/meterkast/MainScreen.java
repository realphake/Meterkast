...
public class MainScreen extends Activity {
	...
	private void drawBarGraphs() {
		...
		// SEE THAT? THAT'S WHY COMMENTS ARE BAD. COMMENTS ROT.
		// So no, I don't make the data do it anymore.
	...
	private void graphRecordings() {
		Bitmap graph = BitmapFactory.decodeResource(getResources(), R.drawable.grid);
		graph = Bitmap.createScaledBitmap(graph, WIDTH, HEIGHT, false);
		graph = drawGraph(WIDTH, HEIGHT, graph); // the information is right here!
	...
	public Bitmap drawGraph(int width, int height, Bitmap graph) {
		...
		for (int i = 1; i <= noRecs; i++) {
			...
			if( !data.singleRecording() ) {
				...
				if (thisRecordingFRST > highestRecorded) {
					// Keep updating the values. 
					highestRecorded = thisRecordingFRST;
				} 
			}
		...
		for (int i = 2; i <= noRecs; i++) { // Start at two because we consider recordings in pairs, each with its preceding.
			...
			int thisRecordingFRST = data.getRecording(i, "frst");
			int thisRecordingSCND = data.getRecording(i,"scnd");
			int thisRecordingCOMBO = thisRecordingFRST+thisRecordingSCND;
			int iPrev = (i - 1);
			int previousRecordingFRST = data.getRecording(iPrev ,"frst");
			int previousRecordingSCND = data.getRecording(iPrev , "scnd");
			int previousRecordingCOMBO = previousRecordingFRST+previousRecordingSCND;
			...
			double previousRecFractionCOMBO = (((double) previousRecordingCOMBO - (double) lowestRecorded) / recordingSpan);
			...
			double thisRecFractionCOMBO = (((double) thisRecordingCOMBO - (double) lowestRecorded) / recordingSpan);
			...
			int drawY1COMBO = (int) (height - (height * previousRecFractionCOMBO));
			int drawY2FRST = (int) (height - (height * thisRecFractionFRST));
			int drawY2SCND = (int) (height - (height * thisRecFractionSCND));
			int drawY2COMBO = (int) (height - (height * thisRecFractionCOMBO));

			// Now then, drawing:
			Canvas canvas = new Canvas(graph);
			Paint p = new Paint();
			if( !data.singleRecording() ) { // Only if there actually is a second recording.
